...
var MAX_KILLS=30
func _ready():
	get_node("Killed/RichTextLabel").text= "0/30"
...
func add_kill():
	numOfKilled += 1
	get_node("Killed/RichTextLabel").text = str(numOfKilled)+"/30"
...
func next_level():
	...
	$AirCraft4.queue_free()
	$AirCraft5.queue_free()
	$AirCraft6.queue_free()
	$AirCraft7.queue_free()
